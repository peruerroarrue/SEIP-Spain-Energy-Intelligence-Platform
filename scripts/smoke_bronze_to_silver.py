...
Run: python scripts/smoke_bronze_to_silver.py
"""

from __future__ import annotations

from seip.ingestion.spark_session import build_local_spark_session
from seip.transform.bronze_to_silver import run
...
def main() -> None:
    spark = build_local_spark_session("seip-bronze-to-silver-smoke")
    spark.sparkContext.setLogLevel("ERROR")

    print("-- running Bronze -> Silver (availableNow) --")
    run(spark, BRONZE_PATH, SILVER_PATH, CHECKPOINT_PATH)
