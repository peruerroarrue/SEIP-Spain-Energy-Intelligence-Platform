...
- [ ] **Cobertura limitada, pendiente de decidir:** el Silver de REData solo cubre **2 días reales** (nunca se hizo un backfill histórico de REData equivalente al de ESIOS) — de las 4.320 horas del feature store, solo 48 (2 días × 24h) tienen `renewable_share` asignado. El join en sí está verificado como correcto; falta decidir si merece la pena un backfill histórico de REData (extender `batch_job.py` o crear uno nuevo, similar a `historical_backfill.py`) para tener cobertura completa de 6 meses, dado que es una feature de segunda prioridad
